...
### Track 2.2 — Streaming Chat Endpoint (Completed: 2026-05-22)
- **Chat API (`POST /api/chat`):** Thin TanStack Start server route using `createFileRoute` with `server.handlers.POST` (streaming Response incompatible with `createServerFn` JSON serialization). Decoupled handler at `src/lib/server/chat.ts` loads encrypted provider settings from DB via `loadSettings`, validates API key presence, constructs OpenAI-compatible client via `createOpenAI`, calls `streamText` with executive resume writer system prompt, returns streaming Response via `result.toUIMessageStreamResponse()`.
- **Error Handling:** 400 `PROVIDER_NOT_CONFIGURED` when no API key configured; 502 `PROVIDER_UNAVAILABLE` when LLM provider unreachable. Consistent with existing route pattern (dynamic imports, decoupled handlers).
- **ChatPanel Component:** Built with AI SDK v6 `useChat` hook at `src/components/ChatPanel.tsx`. Uses `sendMessage({ text })` API with local `useState` for input management. Features: scrollable message list (AI left-aligned, user right-aligned), streaming cursor animation, auto-scroll to latest message, empty state with welcome message ("I'll be your executive resume writer..."), error banner with "Retry" button (includes recovery guidance per Product Guidelines), missing provider placeholder with "Open Settings" wired to `ProviderSettingsContext`, and "Done — extract this section" placeholder button for Track 2.3. Accessible: `aria-live="polite"` region for screen reader announcements, keyboard-navigable input.
- **CV Builder Integration:** Responsive split layout — ChatPanel (top on narrow, left at 2/5 on wide) above the manual CV editor form. All existing form functionality (save, offline, collapse) unchanged.
- **System Prompt:** "You are an executive resume writer helping a professional build their CV. Guide the user section by section — start with Contact, then Executive Summary, Experience, Education, Skills, and Projects. Ask one question at a time. Be encouraging and professional. After each section, suggest when to click 'Done — extract this section'."
- **Testing:** 41 track-specific tests — 5 handler unit tests (streaming, 400, 502, DB injection, system prompt), 9 ChatPanel tests (all UI states), 3 cv-builder integration tests, plus extended existing test file. 284 total project tests passing across 43 files. Chat handler at 100% statement coverage, ChatPanel at 93.1%.
- **Deviations:** Uses `result.toUIMessageStreamResponse()` instead of `createUIMessageStreamResponse()` — equivalent convenience method on `StreamTextResult`. Uses `sendMessage({ text })` instead of deprecated `handleSubmit`/`handleInputChange` pattern. Dynamic import of `DatabaseManager` inside handler following existing route patterns.
